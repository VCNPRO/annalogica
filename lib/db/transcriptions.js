...
export async function saveTranscriptionResults(jobId, data) {
  // 🔥 FIX: Convertir duration a integer (columna es INTEGER en BD)
  const durationInteger = data.duration ? Math.floor(data.duration) : null;

  // 🔥 FIX: Hacer merge de metadata para preservar actions y summaryType
  // Obtener metadata actual del job para hacer merge
  const currentJobResult = await sql`SELECT metadata FROM transcription_jobs WHERE id = ${jobId}`;
  const currentMetadata = currentJobResult.rows[0]?.metadata || {};

  // Merge: preservar propiedades existentes (actions, summaryType) y agregar nuevas
  const metadataToSave = { ...currentMetadata, ...(data.metadata || {}) };
  ...
  const result = await sql`
    ...
    SET
      ...
      tags = ${data.tags || []},
      audio_duration_seconds = ${durationInteger},
