...
// VERSIÓN ROBUSTA: evita "Vercel Blob: body is required" y endurece IO de Blob/Deepgram/OpenAI.
...
// ---------- HELPERS I/O SEGURAS (Blob) ----------
/**
 * Sube texto a Blob **siempre** con body válido (Blob),
 * evitando el error "Vercel Blob: body is required".
 */
const saveTextToFile = async (text: string, baseFilename: string, extension: string) => {
  ...
  const blob = await put(
    ...
    body,
    {
      ...
      // en tu proyecto usas token; mantenemos compatibilidad
...
/** Eliminación defensiva en Blob (usa token y no revienta si la URL no es de Blob) */
const safeDeleteFromBlob = async (maybeBlobUrl: string) => {
  try {
    // Solo intentamos del si parece una URL de Vercel Blob
...
// ---------- FUNCIONES DE AUDIO (DEEPGRAM) ----------
export const transcribeFile = inngest.createFunction(
  // límites ajustados a plan y robustez
  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      // (Opcional) Identidad de hablantes con OpenAI
      ...
      await step.run('update-db-with-results', async () => {
        const speakers = [...new Set(utterances.map((u: any) => `Hablante ${u.speaker}`))].sort();
        const metadata = { ...jobMetadata, speakers, speakerIdentities };
        await TranscriptionJobDB.updateResults(jobId, {
          assemblyaiId: deepgramResult.metadata.request_id, // Nota: es el request_id de Deepgram; conservamos tu clave
          txtUrl,
          audioDuration,
          metadata,
          srtUrl,
      ...
      // Limpieza del original (segura con token y patrón de URL)
      await step.run('delete-original-audio', async () => {
        await safeDeleteFromBlob(audioUrl);
      });
      ...
      if (actions.includes('Resumir') || actions.includes('Etiquetas')) {
        // API moderna de Inngest: id del step + payload del evento
        await step.sendEvent('trigger-summarization', {
          name: 'task/summarize',
          data: { jobId, actions }
        });
      } else {
