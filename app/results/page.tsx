...
export default function Results() {
  ...
  const loadFiles = async () => {
    try {
      const response = await fetch(${API_URL}/files, { headers: { 'Authorization': Bearer ${localStorage.getItem('token')} }});
  ...
  const downloadFile = async (key) => {
    try {
      const response = await fetch(${API_URL}/download?key=${encodeURIComponent(key)}, { headers: { 'Authorization': Bearer ${localStorage.getItem('token')} }});
      const data = await response.json();
  ...
  const deleteFile = async (fileName) => {
    ...
    try {
      await fetch(${API_URL}/files/${encodeURIComponent(fileName)}, { method: 'DELETE', headers: { 'Authorization': Bearer ${localStorage.getItem('token')} }});
      loadFiles();
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        {files.length === 0 ? <p className="text-gray-500">No hay archivos</p> : (
